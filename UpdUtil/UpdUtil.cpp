...
void Execute(std::wstring wFile, std::wstring wParams)
{
	...
	std::wcout << wFile.c_str() << si.lpParameters << std::endl;
...
void PrintUsage() 
{
	std::wcout << L"MajorPrivacy Update Utility - Usage" << std::endl;
	...
	_In_ int       nCmdShow)
{
	...
	*wcsrchr(szPath, L'\\') = L'\0';
	...
	// install major-privacy /version:1.6.1 /path:C:\Projects\MajorPrivacy\x64\Debug\MajorPrivacy_test
	...
	if (!HasFlag(arguments, L"embedded")) {
		...
		if (HasFlag(arguments, L"pause")) {
			std::cout << "MajorPrivacy Update Utility" << std::endl;
	...
	if (temp_dir.empty()) {
		...
		temp_dir = std::wstring(szTemp) + L"MajorPrivacy-Updater";
	}
	...
	else if ((arguments.size() >= 2 && (arguments[0] == L"update" || arguments[0] == L"upgrade" || arguments[0] == L"install")))
	{
		...
		if (!wOpen.empty()) {
			Execute(base_dir + L"\\MajorPrivacy.exe", L"");
		}
	...
	else if (arguments.size() >= 2 && (arguments[0] == L"get_cert" || arguments[0] == L"get_cert_lr"))
	{
		...
		if (arguments[0] == L"get_cert_lr") 
		{
			...
			if(!serial.empty()) // serial is optional uodate key is used instead
				path += L"&SN=" + serial;
		}
		else
		{
