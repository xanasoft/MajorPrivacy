...
_CryptCATAdminCalcHashFromFileHandle2 P_CryptCATAdminCalcHashFromFileHandle2;
_CryptCATAdminAcquireContext P_CryptCATAdminAcquireContext;
_CryptCATAdminAcquireContext2 P_CryptCATAdminAcquireContext2;
_CryptCATAdminEnumCatalogFromHash P_CryptCATAdminEnumCatalogFromHash;
_CryptCATCatalogInfoFromContext P_CryptCATCatalogInfoFromContext;
_CryptCATAdminReleaseCatalogContext P_CryptCATAdminReleaseCatalogContext;
_CryptCATAdminReleaseContext P_CryptCATAdminReleaseContext;
//_WTHelperProvDataFromStateData P_WTHelperProvDataFromStateData_I;
//_WTHelperGetProvSignerFromChain P_WTHelperGetProvSignerFromChain_I;
_WinVerifyTrust P_WinVerifyTrust_I;
_CertNameToStr P_CertNameToStr_I;
_CertGetEnhancedKeyUsage P_CertGetEnhancedKeyUsage_I;
_CertDuplicateCertificateContext P_CertDuplicateCertificateContext_I;
_CertFreeCertificateContext P_CertFreeCertificateContext_I;
    ...
    )
{
    ...
    if (wintrust)
    {
        P_CryptCATAdminCalcHashFromFileHandle = PhGetDllBaseProcedureAddress(wintrust, "CryptCATAdminCalcHashFromFileHandle", 0);
        P_CryptCATAdminCalcHashFromFileHandle2 = PhGetDllBaseProcedureAddress(wintrust, "CryptCATAdminCalcHashFromFileHandle2", 0);
        P_CryptCATAdminAcquireContext = PhGetDllBaseProcedureAddress(wintrust, "CryptCATAdminAcquireContext", 0);
        P_CryptCATAdminAcquireContext2 = PhGetDllBaseProcedureAddress(wintrust, "CryptCATAdminAcquireContext2", 0);
        P_CryptCATAdminEnumCatalogFromHash = PhGetDllBaseProcedureAddress(wintrust, "CryptCATAdminEnumCatalogFromHash", 0);
        P_CryptCATCatalogInfoFromContext = PhGetDllBaseProcedureAddress(wintrust, "CryptCATCatalogInfoFromContext", 0);
        P_CryptCATAdminReleaseCatalogContext = PhGetDllBaseProcedureAddress(wintrust, "CryptCATAdminReleaseCatalogContext", 0);
        P_CryptCATAdminReleaseContext = PhGetDllBaseProcedureAddress(wintrust, "CryptCATAdminReleaseContext", 0);
        //P_WTHelperProvDataFromStateData_I = PhGetDllBaseProcedureAddress(wintrust, "WTHelperProvDataFromStateData", 0);
        //P_WTHelperGetProvSignerFromChain_I = PhGetDllBaseProcedureAddress(wintrust, "WTHelperGetProvSignerFromChain", 0);
        P_WinVerifyTrust_I = PhGetDllBaseProcedureAddress(wintrust, "WinVerifyTrust", 0);
    }

    if (crypt32)
    {
        P_CertNameToStr_I = PhGetDllBaseProcedureAddress(crypt32, "CertNameToStrW", 0);
        P_CertGetEnhancedKeyUsage_I = PhGetDllBaseProcedureAddress(crypt32, "CertGetEnhancedKeyUsage", 0);
        P_CertDuplicateCertificateContext_I = PhGetDllBaseProcedureAddress(crypt32, "CertDuplicateCertificateContext", 0);
        P_CertFreeCertificateContext_I = PhGetDllBaseProcedureAddress(crypt32, "CertFreeCertificateContext", 0);
    }

    if (
        P_CryptCATAdminCalcHashFromFileHandle &&
        P_CryptCATAdminAcquireContext &&
        P_CryptCATAdminEnumCatalogFromHash &&
        P_CryptCATCatalogInfoFromContext &&
        P_CryptCATAdminReleaseCatalogContext &&
        P_CryptCATAdminReleaseContext &&
        P_WinVerifyTrust_I &&
        P_CertNameToStr_I &&
        P_CertDuplicateCertificateContext_I &&
        P_CertFreeCertificateContext_I
        )
    {
    ...
    )
{
    ...
    if (numberOfSignatures != 0)
    {
        ...
        {
            if (sgnr->csCertChain != 0)
                signatures[index++] = (PCERT_CONTEXT)P_CertDuplicateCertificateContext_I(sgnr->pasCertChain[0].pCert);
    ...
    )
{
    ...
    status = P_WinVerifyTrust_I(INVALID_HANDLE_VALUE, ActionId, &trustData);
    ...
    P_WinVerifyTrust_I(INVALID_HANDLE_VALUE, ActionId, &trustData);
    ...
    )
{
    ...
    if (P_CryptCATAdminAcquireContext2)
    {
        if (!P_CryptCATAdminAcquireContext2(&catAdminHandle, &DriverActionVerify, HashAlgorithm, NULL, 0))
            return FALSE;
    }
    else
    {
        if (!P_CryptCATAdminAcquireContext(&catAdminHandle, &DriverActionVerify, 0))
            return FALSE;
    }
    ...
    if (P_CryptCATAdminCalcHashFromFileHandle2)
    {
        if (!P_CryptCATAdminCalcHashFromFileHandle2(catAdminHandle, FileHandle, &fileHashLength, fileHash, 0))
        {
            PhFree(fileHash);
            fileHash = PhAllocate(fileHashLength);
            ...
            if (!P_CryptCATAdminCalcHashFromFileHandle2(catAdminHandle, FileHandle, &fileHashLength, fileHash, 0))
            {
                P_CryptCATAdminReleaseContext(catAdminHandle, 0);
                PhFree(fileHash);
                return FALSE;
            }
        }
    ...
    {
        if (!P_CryptCATAdminCalcHashFromFileHandle(FileHandle, &fileHashLength, fileHash, 0))
        {
            PhFree(fileHash);
            fileHash = PhAllocate(fileHashLength);
    ...
    )
{
    ...
    if (P_CryptCATAdminCalcHashFromFileHandle2)
    {
        if (!P_CryptCATAdminCalcHashFromFileHandle2(catAdminHandle, FileHandle, FileHashLength, FileHashBuffer, 0))
        {
            P_CryptCATAdminReleaseContext(catAdminHandle, 0);
            return FALSE;
        }
    ...
    {
        if (!P_CryptCATAdminCalcHashFromFileHandle(FileHandle, FileHashLength, FileHashBuffer, 0))
        {
            P_CryptCATAdminReleaseContext(catAdminHandle, 0);
            return FALSE;
        }
    ...
    )
{
    ...
    *NumberOfSignatures = 0;
    ...
    catInfoHandle = P_CryptCATAdminEnumCatalogFromHash(
        catAdminHandle,
        fileHash,
        fileHashLength,
        0,
    ...
    if (catInfoHandle)
    {
        ...
        if (P_CryptCATCatalogInfoFromContext(catInfoHandle, &ci, 0))
        {
            ...
            if (verInfo.pcSignerCertContext)
                P_CertFreeCertificateContext_I(verInfo.pcSignerCertContext);
        }

        P_CryptCATAdminReleaseCatalogContext(catAdminHandle, catInfoHandle, 0);
    }
    else
    {
    ...
    )
{
    ...
    {
        if (P_CryptCATAdminAcquireContext2 && P_CryptCATAdminCalcHashFromFileHandle2)
        {
            PhFreeVerifySignatures(signatures, numberOfSignatures);
    ...
    )
{
    ...
    if (Signatures)
    {
        for (i = 0; i < NumberOfSignatures; i++)
            P_CertFreeCertificateContext_I(Signatures[i]);
    ...
    )
{
    ...
    if (!P_CertGetEnhancedKeyUsage_I(Certificate, CERT_FIND_EXT_ONLY_ENHKEY_USAGE_FLAG, usage, &usageLength))
    {
        assert(FALSE);
        return FALSE;
    }
    ...
    if (SignerName)
    {
        if (numberOfSignatures != 0)
        {
            *SignerName = PhGetSignerNameFromCertificate(signatures[0]);
            ...
            BYTE sha1Hash[20];
            DWORD sha1HashSize = sizeof(sha1Hash);
            if (SignerSHA1Hash && CertGetCertificateContextProperty(signatures[0], CERT_SHA1_HASH_PROP_ID, sha1Hash, &sha1HashSize) && *SignerSHA1HashSize >= sha1HashSize) {
                memcpy(SignerSHA1Hash, sha1Hash, sha1HashSize);
                *SignerSHA1HashSize = sha1HashSize;
            }

            // Get SHA-256 fingerprint
            BYTE sha256Hash[32];
            DWORD sha256HashSize = sizeof(sha256Hash);
            if (SignerSHAXHash && CertGetCertificateContextProperty(signatures[0], CERT_SHA256_HASH_PROP_ID, sha256Hash, &sha256HashSize) && *SignerSHAXHash >= sha256HashSize) {
                memcpy(SignerSHAXHash, sha256Hash, sha256HashSize);
                *SignerSHAXHashSize = sha256HashSize;
            }

            /*// Compute SHA-384 fingerprint
            BYTE sha384Hash[48];
            DWORD sha384HashSize = sizeof(sha384Hash);
            if (CryptHashCertificate(
                NULL,
                CALG_SHA_384,
                0,
                signatures[0]->pbCertEncoded,
                signatures[0]->cbCertEncoded,
                sha384Hash,
                &sha384HashSize))
            {
            ...
            // Compute SHA-384 fingerprint
            BYTE sha512Hash[64];
            DWORD sha512HashSize = sizeof(sha512Hash);
            if (CryptHashCertificate(
                NULL,
                CALG_SHA_512,
                0,
    ...
    )
{
...

    {
        ...
        if (entry = PhFindEntryHashtable(PhpVerifyCacheHashTable, &lookupEntry))
        {
			...
			if (SignerSHA1Hash && SignerSHA1HashSize && entry->SignerSHA1HashSize && *SignerSHA1HashSize >= entry->SignerSHA1HashSize)
			{
				memcpy(SignerSHA1Hash, entry->SignerSHA1Hash, entry->SignerSHA1HashSize);
				*SignerSHA1HashSize = entry->SignerSHA1HashSize;
			}

            if (SignerSHAXHash && SignerSHAXHashSize && entry->SignerSHAXHashSize && *SignerSHAXHashSize >= entry->SignerSHAXHashSize)
            {
                memcpy(SignerSHAXHash, entry->SignerSHAXHash, entry->SignerSHAXHashSize);
                *SignerSHAXHashSize = entry->SignerSHAXHashSize;
            }
...

    {
        ...
        if (!CachedOnly)
        {
            ...
            info.Flags = PH_VERIFY_PREVENT_NETWORK_ACCESS;
            info.FileHandle = fileHandle;
            result = PhVerifyFileWithAdditionalCatalog(&info, PackageFullName, &signerName, signerSHA1Hash, &signerSHA1HashSize, signerSHAXHash, &signerSHAXHashSize);
        ...
        if (!CachedOnly) // if (result != VrUnknown)
        {
			...
			newEntry.SignerSHA1HashSize = signerSHA1HashSize;
            if(signerSHA1HashSize)
				memcpy(newEntry.SignerSHA1Hash, signerSHA1Hash, signerSHA1HashSize);
            newEntry.SignerSHAXHashSize = signerSHAXHashSize;
            if(signerSHAXHashSize)
                memcpy(newEntry.SignerSHAXHash, signerSHAXHash, signerSHAXHashSize);
        ...
        if (SignerSHA1Hash && SignerSHA1HashSize && signerSHA1HashSize && *SignerSHA1HashSize >= signerSHA1HashSize)
        {
            memcpy(SignerSHA1Hash, signerSHA1Hash, signerSHA1HashSize);
            *SignerSHA1HashSize = signerSHA1HashSize;
        }

        if (SignerSHAXHash && SignerSHAXHashSize && signerSHAXHashSize && *SignerSHAXHashSize >= signerSHAXHashSize)
        {
            memcpy(SignerSHAXHash, signerSHAXHash, signerSHAXHashSize);
            *SignerSHAXHashSize = signerSHAXHashSize;
        }
