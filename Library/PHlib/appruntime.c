...
VOID PhQueryApplicationModelPackageInformation(
    _In_ void* pAppPackage,
    _In_ void* pAppPackage2,
    _Out_opt_ PPH_STRING *packageInstallLocationString,
    _Out_opt_ PPH_STRING *packageNameString,
    _Out_opt_ PPH_STRING *packageFullNameString,
    _Out_opt_ PPH_STRING *packageFamilyNameString,
    _Out_opt_ PPH_STRING *packageDisplayNameString,
    _Out_opt_ PPH_STRING *packageLogoString,
    _Out_opt_ PPH_STRING *packageVersionString
    )
{
    __x_ABI_CWindows_CApplicationModel_CIPackage* AppPackage = pAppPackage;
    __x_ABI_CWindows_CApplicationModel_CIPackage2* AppPackage2 = pAppPackage2;
    ...
    if (packageInstallLocationString)
    {
...
BOOLEAN PhAddApplicationModelPackageInformation(
    ...
    )
{
    ...
    PhQueryApplicationModelPackageInformation(AppPackage, AppPackage2,
        FlagOn(FilterType, PH_QUERY_PACKAGE_INFO_LOCATION) ? &packageInstallLocationString : NULL,
        FlagOn(FilterType, PH_QUERY_PACKAGE_INFO_NAME) ? &packageNameString : NULL,
        &packageFullNameString, // required PH_QUERY_PACKAGE_INFO_FULLNAME
        FlagOn(FilterType, PH_QUERY_PACKAGE_INFO_FAMILYNAME) ? &packageFamilyNameString : NULL,
        FlagOn(FilterType, PH_QUERY_PACKAGE_INFO_DISPLAYNAME) ? &packageDisplayNameString : NULL,
        FlagOn(FilterType, PH_QUERY_PACKAGE_INFO_LOGO) ? &packageLogoString : NULL,
        FlagOn(FilterType, PH_QUERY_PACKAGE_INFO_VERSION) ? &packageVersionString : NULL);
...
VOID PhEnumPackageApplicationUserModelIdsCB(
    _In_opt_ const wchar_t* packageFamilyName,
    _In_ BOOLEAN (*callBack)(
        _In_ PVOID param,
        _In_ void* AppPackage,
        _In_ void* AppPackage2
        ),
    _In_ PVOID param
    )
{
    ...
    if (packageFamilyName)
    {
        ...
        PhCreateWindowsRuntimeStringReference(packageFamilyName, &string);

        if (PhGetOwnTokenAttributes().Elevated)
            status = __x_ABI_CWindows_CManagement_CDeployment_CIPackageManager_FindPackagesByPackageFamilyName(packageManager, &string, &enumPackages);
        else
            status = __x_ABI_CWindows_CManagement_CDeployment_CIPackageManager_FindPackagesByUserSecurityIdPackageFamilyName(packageManager, NULL, &string, &enumPackages);
    }
    else
    {
        if (PhGetOwnTokenAttributes().Elevated)
    ...
    while (haveCurrentPackage)
    {
        ...
        if (HR_SUCCESS(status))
        {
            ...
            if (HR_SUCCESS(status))
            {
                if (!callBack(param, currentPackage, currentPackage2))
                    status = S_FALSE;
...
BOOLEAN PhEnumPackageApplicationUserModelIds_callBack(
    _In_ PVOID param,
    _In_ void* AppPackage,
    _In_ void* AppPackage2
    )
{
    ...
    PhAddApplicationModelPackageInformation(
        list,
        AppPackage,
        AppPackage2,
        PH_QUERY_PACKAGE_INFO_ALL
        //PH_QUERY_PACKAGE_INFO_NAME | PH_QUERY_PACKAGE_INFO_FULLNAME | PH_QUERY_PACKAGE_INFO_FAMILYNAME | PH_QUERY_PACKAGE_INFO_VERSION
        // | PH_QUERY_PACKAGE_INFO_DISPLAYNAME | PH_QUERY_PACKAGE_INFO_LOGO // these are extra slow
        );
    ...
    )
{
    ...
    PhEnumPackageApplicationUserModelIdsCB(NULL, PhEnumPackageApplicationUserModelIds_callBack, list);
