...
void CPipeServer::ClientConnected(const PPipeSocket& pPipe)
{
...
void CPipeServer::RunClientThread(SPipeClient* pClient)
{
    ...
    GetNamedPipeClientProcessId(pClient->pPipe->hPipe, &ClientPid);
...
//#endif
        {
            ...
            if (I != m_MessageHandlers.end())
                status = I->second(in_msg->MessageId, &recvBuff, &sendBuff, ClientPid, -1);   
...
int CPipeServer::BroadcastMessage(uint32 msgId, const CBuffer* msg)
{
...
STATUS SPipeSocket::ReadPacket(CBuffer& recvBuff)
{
    do
    {
        ...
        if (recvBuff.GetLengthLeft() < PIPE_BUFSIZE)
            recvBuff.SetSize(recvBuff.GetSize(), true, max(PIPE_BUFSIZE, recvBuff.GetLengthLeft() + recvBuff.GetLength()));
