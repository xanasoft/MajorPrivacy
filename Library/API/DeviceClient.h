...
public:
    ...
    virtual STATUS Call(const CBuffer& inBuff, CBuffer& outBuff)
    {
        ...
        IO_STATUS_BLOCK IoStatusBlock;
        NTSTATUS status = NtDeviceIoControlFile(m_IoDeviceHandle, NULL, NULL, NULL, &IoStatusBlock, API_DEVICE_CTLCODE, (PVOID)inBuff.GetBuffer(), (ULONG)inBuff.GetSize(), outBuff.GetBuffer(), (ULONG)outBuff.GetLength());
        if (!NT_SUCCESS(status))
            return ERR(status);
