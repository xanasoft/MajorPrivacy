...
static const WCHAR *File_Mup = L"\\device\\mup\\";
static const ULONG File_MupLen = 12;

static const WCHAR *File_BQQB = L"\\??\\";


#define FILE_SHARE_VALID_FLAGS          0x00000007

CPathReparse* CPathReparse::m_pInstance = NULL;

#define MAX_DRIVES 26


CPathReparse::CPathReparse()
{
...
CPathReparse* CPathReparse::Instance()
{
    // todo single init
	if (!m_pInstance)
		m_pInstance = new CPathReparse;
	return m_pInstance;
}

void CPathReparse::Dispose()
{
    if (m_pInstance)
    {
        delete m_pInstance;
...
std::wstring CPathReparse::GetNtPathFromHandle(HANDLE hFile)
{
...
bool CPathReparse::InitDrives(ULONG DriveMask)
{
    NTSTATUS status;
...
void CPathReparse::AdjustDrives(ULONG path_drive_index, bool subst, const std::wstring& path)
{
...
ULONG CPathReparse::GetVolumeSN(const std::wstring& DriveNtPath)
{
    ULONG sn = 0;
    HANDLE handle;
...
std::wstring CPathReparse::GetDrivePath(ULONG DriveIndex)
{
...
std::shared_ptr<const CPathReparse::SDrive> CPathReparse::GetDriveForPath(const std::wstring& Path)
{
...
std::shared_ptr<const CPathReparse::SDrive> CPathReparse::GetDriveForUncPath(const std::wstring& Path, ULONG *OutPrefixLen)
{
    std::shared_ptr<CPathReparse::SDrive> drive;
    WCHAR *ptr1a;
...
std::shared_ptr<const CPathReparse::SDrive> CPathReparse::GetDriveForLetter(WCHAR drive_letter)
{
    std::shared_ptr<const CPathReparse::SDrive> drive;
...
void CPathReparse::InitLinksLocked()
{
    NTSTATUS status;
    OBJECT_ATTRIBUTES objattrs;
    UNICODE_STRING objname;
...
std::wstring CPathReparse::GetName_TranslateSymlinks(const std::wstring& Path, bool *translated)
{
...
bool CPathReparse::AddLink(bool PermLink, const std::wstring& Src, const std::wstring& Dst)
{
    ULONG src_len;
    ULONG dst_len;
    ULONG drive_letter;
...
void CPathReparse::RemovePermLinks(const std::wstring& path)
{
    ULONG path_len;
    std::shared_ptr<SLink> old_link;
...
ULONG CPathReparse::GetDrivePrefixLength(const std::wstring& work_str)
{
    ULONG prefix_len = 0;
...
std::wstring CPathReparse::TranslateTempLinks(const std::wstring& _TruePath, bool bReverse, bool StripLastPathComponent)
{
...
std::wstring CPathReparse::TranslateTempLinks_2(const std::wstring& input_str, bool bReverse)
{
    std::shared_ptr<SLink> link;
...
std::shared_ptr<CPathReparse::SLink> CPathReparse::AddTempLink(const std::wstring& path)
{
    NTSTATUS status;
    HANDLE handle;
    IO_STATUS_BLOCK IoStatusBlock;
...
std::shared_ptr<CPathReparse::SLink> CPathReparse::AddReverseTempLink(const std::wstring& path)
{
    std::shared_ptr<SLink> link;
...
std::wstring CPathReparse::FixPermLinksForTempLink(const std::wstring& name, bool bReverse)
{
...
void CPathReparse::GetDriveAndLinkForPath(const std::wstring& Path, std::shared_ptr<const SDrive>& OutDrive, std::shared_ptr<const SLink>& OutLink)
{
...
std::shared_ptr<CPathReparse::SLink> CPathReparse::FindPermLinksForMatchPath(const std::wstring& name, bool bReverse)
{
    std::unique_lock lock(m_Mutex);

    for( auto I = m_PermLinks.begin(); I != m_PermLinks.end(); ++I)
    {
        std::shared_ptr<CPathReparse::SLink> link = *I;
...
std::wstring CPathReparse::FixPermLinksForMatchPath(const std::wstring& name)
{
    std::shared_ptr<CPathReparse::SLink> link = FindPermLinksForMatchPath(name, false);
    if (link)
    {
...
std::shared_ptr<const CPathReparse::SGuid> CPathReparse::GetGuidForPath(const std::wstring& Path)
{
    std::unique_lock lock(m_Mutex);

    for(auto I = m_GuidLinks.begin(); I != m_GuidLinks.end(); ++I)
    {
        std::shared_ptr<CPathReparse::SGuid> guid = *I;
...
std::shared_ptr<const CPathReparse::SGuid> CPathReparse::GetLinkForGuid(const std::wstring& guid_str)
{
    std::unique_lock lock(m_Mutex);
...
std::wstring CPathReparse::TranslateGuidToNtPath(const std::wstring& GuidPath)
{
    //
    // guid path L" \\??\\Volume{...}\..."
    //
    ...
    {
        std::shared_ptr<const CPathReparse::SGuid> guid = GetLinkForGuid(&GuidPath[10]);
        if (guid)
...
std::wstring CPathReparse::TranslateDosToNtPath(const std::wstring& DosPath, bool bAsIsOnError)
{
    //
    // network path L"\\..."
    //
...
std::wstring CPathReparse::TranslateNtToDosPath(const std::wstring& NtPath, bool bAsIsOnError)
{
    // 
