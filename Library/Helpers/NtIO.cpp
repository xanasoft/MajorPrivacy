...
NTSTATUS NtIo_DeleteFolderRecursivelyImpl(POBJECT_ATTRIBUTES objattrs, bool (*cb)(const WCHAR* info, void* param), void* param)
{
	...
	for ( ; status == STATUS_SUCCESS; )
	{
		...
		PFILE_BOTH_DIR_INFORMATION Info = (PFILE_BOTH_DIR_INFORMATION)malloc(PAGE_SIZE);
...
NTSTATUS NtIo_RenameFileOrFolder(POBJECT_ATTRIBUTES src_objattrs, POBJECT_ATTRIBUTES dest_objattrs, const WCHAR* DestName, ULONG Share, ULONG Create)
{
	...
	ULONG NameLen = (ULONG)wcslen(DestName) * sizeof(WCHAR) + 16;
	ULONG InfoSize = sizeof(FILE_RENAME_INFORMATION) + NameLen;
	...
	pInfo->FileNameLength = (ULONG)wcslen(DestName) * sizeof(WCHAR);
	wcscpy_s(pInfo->FileName, NameLen/ sizeof(WCHAR), DestName);
...
BOOLEAN NtIo_FileExists(POBJECT_ATTRIBUTES objattrs)
{
	...
	HANDLE handle;
	status = NtCreateFile(&handle, SYNCHRONIZE, objattrs, &Iosb, NULL, 0, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, FILE_OPEN, FILE_SYNCHRONOUS_IO_NONALERT | FILE_OPEN_REPARSE_POINT, NULL, 0);
	if (NT_SUCCESS(status))
	{
