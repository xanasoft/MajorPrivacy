...
void* InjectLdr_AllocMemory(HANDLE hProcess, SIZE_T uSize, BOOLEAN bExecutable
#ifdef _M_ARM64
	, BOOLEAN use_arm64ec
#endif
);

void *InjectLdr_CopyCode(HANDLE hProcess, SIZE_T uCodeSize, const void* pCode
#ifdef _M_ARM64
	, BOOLEAN UseArm64ec
#endif
);

BOOLEAN InjectLdr_BuildTramp(UCHAR* pTrampCode, ULONG_PTR TrampAddr, int LongDiff
#ifdef _M_ARM64
	, BOOLEAN UseArm64ec
#endif
);

BOOLEAN InjectLdr_WriteJump(HANDLE hProcess, ULONG_PTR Detour, int LongDiff
#ifdef _M_ARM64
	, BOOLEAN UseArm64ec
#endif
);
...
ULONG InjectLdr_LoadCode(BOOLEAN b64bit, void **ppLdrCode, ULONG *pLdrCodeSize, ULONG *pStartOffset, ULONG* pDataOffset, ULONG* pDetourOffset)
{
	HRSRC res = FindResource(NULL, b64bit ? L"LDRCODE64" : L"LDRCODE32", RT_RCDATA);
	if (! res)
        return 0x10;
    ...
    if (dos_hdr->e_magic == 'MZ' || dos_hdr->e_magic == 'ZM') 
	{
		...
		if (nt_hdrs->Signature != IMAGE_NT_SIGNATURE)   // 'PE\0\0'
			return 0x14;
		if (nt_hdrs->OptionalHeader.Magic != (b64bit ? IMAGE_NT_OPTIONAL_HDR64_MAGIC : IMAGE_NT_OPTIONAL_HDR32_MAGIC)) 
			return 0x15;
...
{
#ifdef _WIN64
	ULONG err = InjectLdr_LoadCode(TRUE, &g_LdrCode_Ptr, &g_LdrCode_Size, &g_LdrCode_Start_Offset, &g_LdrCode_Data_Offset, &g_LdrCode_Detour_Offset);
	if(!err)
		err = InjectLdr_LoadCode(FALSE, &g_LdrCode32_Ptr, &g_LdrCode32_Size, NULL, NULL, &g_LdrCode32_Detour_Offset);
#else
	ULONG err = InjectLdr_LoadCode(FALSE, &g_LdrCode_Ptr, &g_LdrCode_Size, &g_LdrCode_Start_Offset, &g_LdrCode_Data_Offset, &g_LdrCode_Detour_Offset);
#endif
	if (err)
		return err;
	...
	{
		void* func_ptr = GetProcAddress(NtDll, NtdllExports[i]);
		if(!func_ptr) return 0x21 + i;
		memcpy((void*)((ULONG_PTR)g_NtDllSavedCode + (NATIVE_FUNCTION_SIZE * i)), func_ptr, NATIVE_FUNCTION_SIZE);
	}
    ...
    g_LdrInitializeThunk = (ULONG_PTR) GetProcAddress(NtDll, "LdrInitializeThunk");
	...
	g_LdrInitializeThunkEC = (ULONG_PTR)NtDll + FindDllExportFromFile(path, "#LdrInitializeThunk");

	HMODULE KernelBaseDll = GetModuleHandleW(L"kernelbase.dll");
	__sys_VirtualAlloc2 = (P_VirtualAlloc2)GetProcAddress(KernelBaseDll, "VirtualAlloc2");

#elif _WIN64
	//if (OsBuild >= 10000) { // Win 10
		unsigned char* code;
...
ULONG InjectLdr(HANDLE hProcess, ULONG uFlags)
{
	...
	if ((!g_LdrCode_Ptr) || (!g_LdrCode_ExtraData)
#ifdef _M_ARM64
			|| (LdrData.Flags.IsArm64ec && ((!g_LdrInitializeThunkEC) || (!__sys_VirtualAlloc2)))
#endif
	)
		return 0x52;
	...
	LdrData.RealNtDeviceIoControlFile = (ULONG64)GetProcAddress((HMODULE)LdrData.NtDllBase, "NtDeviceIoControlFile");
	LdrData.NativeNtProtectVirtualMemory = (ULONG64)GetProcAddress((HMODULE)LdrData.NtDllBase, "NtProtectVirtualMemory");
	LdrData.NativeNtRaiseHardError = (ULONG64)GetProcAddress((HMODULE)LdrData.NtDllBase, "NtRaiseHardError");
	
	//
	// Copy the Native LdrCode into the target process
	//

	void *pLdrCodeAddr = InjectLdr_CopyCode(hProcess, g_LdrCode_Size, g_LdrCode_Ptr
#ifdef _M_ARM64
		, (BOOLEAN)LdrData.Flags.IsArm64ec
#endif
	);
	if (!pLdrCodeAddr)
		return 0x54;
...
#ifdef _M_ARM64
	int LongDiff = 2; // on ARM64 always 64-bit absolute jump
#else
	int LongDiff = 1; // default 32-bit absolute jump
	...
	if (LdrData.Flags.IsWoW64) 
	{
		...
		void *pLdrCode32Addr = InjectLdr_CopyCode(hProcess, g_LdrCode32_Size, g_LdrCode32_Ptr
#ifdef _M_ARM64
			, FALSE
#endif
		);
	...
	if (!InjectLdr_BuildTramp(LdrData.LdrInitializeThunk_Tramp, TrampAddr, LongDiff
#ifdef _M_ARM64
		, (BOOLEAN)LdrData.Flags.IsArm64ec
#endif
	))
	...
	void *pExtraDataAddress = InjectLdr_AllocMemory(hProcess, g_LdrCode_ExtraData_Size , FALSE
#ifdef _M_ARM64
		, FALSE
#endif
	);
	...
	if (!InjectLdr_WriteJump(hProcess, (ULONG_PTR)((UCHAR *)pLdrCodeAddr + g_LdrCode_Start_Offset), LongDiff
#ifdef _M_ARM64
		, (BOOLEAN)LdrData.Flags.IsArm64ec
#endif
	))
...
void* InjectLdr_AllocMemory(HANDLE hProcess, SIZE_T uSize, BOOLEAN bExecutable
	...
	if (UseArm64ec && bExecutable) {
		...
		{
			pAllocAddr = __sys_VirtualAlloc2(hProcess, (void*)BaseAddr, uAllocSize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE, &Parameter, 1);
		}
	} 
	else
#endif
...
void *InjectLdr_CopyCode(HANDLE hProcess, SIZE_T uCodeSize, const void* pCode
	...
	void* pAllocAddr = InjectLdr_AllocMemory(hProcess, uCodeSize, TRUE
#ifdef _M_ARM64
		, UseArm64ec
#endif
	);
...
#else
	*xCode.pB++ = 0xE9;			// jmp ; relative
	*xCode.pD++ = (ULONG)(g_LdrInitializeThunk + uOffset - (TrampAddr + uOffset + 5)); // jump offset
#endif
#endif
...
#else
	*xCode.pB++ = 0xE9; // jmp ; relative
	*xCode.pD++ = (ULONG)(Detour - (g_LdrInitializeThunk + 5));
	// 5
#endif
