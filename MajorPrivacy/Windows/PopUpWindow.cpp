...
void CPopUpWindow::PushFwRuleEvent(EConfigEvent EventType, const CFwRulePtr& pFwRule)
{
	...
	List.append(SFwRuleEvent{ EventType, pFwRule });
...
void CPopUpWindow::OnFwRule()
{
	...
	auto Items = ui.treeFwRules->selectedItems();
	if (Items.isEmpty()) {
	...
	foreach(const SFwRuleEvent& Event, Events)
	{
		...
		else if (pSender == ui.btnFwReject)
		{
			switch (EventType)
			{
			case EConfigEvent::eModified:
...
void CPopUpWindow::OnFwRuleDblClick(QTreeWidgetItem* pItem)
{
	...
	CFirewallRuleWnd* pFirewallRuleWnd = new CFirewallRuleWnd(pFwRule, QSet<CProgramItemPtr>() << pProgram);
