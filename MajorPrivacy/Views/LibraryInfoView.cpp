...
static SCICertificatePtr AddCertificateChain(QTreeWidgetItem* pItem, const SCICertificatePtr& pCert, const QByteArray& Hash = QByteArray())
{
	...
	if(pCert->Issuer)
		AddCertificateChain(pSigner, pCert->Issuer);
...
void CLibraryInfoView::Sync()
{
	...
	for (auto pInfo : Items)
	{
		...
		SEmbeddedCIInfoPtr pEmbeddedInfo = GetEmbeddedCIInfo(filePath);
		if (pEmbeddedInfo)
		{
			pItem->setText(eValid, pEmbeddedInfo->EmbeddedSignatureValid ? tr("Yes") : tr("INVALID"));

			for (auto& Signer : pEmbeddedInfo->EmbeddedSigners)
				FoundCert = AddCertificateChain(pItem, Signer, Hash);
		}
		
		SCatalogCIInfoPtr pCatalogInfo = GetCatalogCIInfo(filePath);
		if (pCatalogInfo && !pCatalogInfo->CatalogSigners.empty())
		{
			...
			for (auto& Catalog : pCatalogInfo->CatalogSigners)
			{
				...
				for (auto& Signer : Catalog.second) {
					SCICertificatePtr FoundCert2 = AddCertificateChain(pCatalog, Signer, Hash);
					if (FoundCert2)
					{
