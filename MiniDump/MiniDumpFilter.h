 ...
 * @param pszDumpPath    Optional directory where the dump should be written
 *                       instead of the Windows temp folder. The directory (and
 *                       any missing parents) is created automatically and a
 *                       timestamped dump file is written inside it. If the
 *                       directory cannot be written, the dumper falls back to
 *                       the temp folder. Pass NULL to always use the temp folder.
 *
 ...
 *
 * Example usage:
 *   MiniDumpFilter_Init(NULL, L"MyApp", MDF_TYPE_TRIAGE, MyCleanupCallback, NULL);
 */
DUMP_EXPORT BOOL MiniDumpFilter_Init(LPCWSTR pszDumperPath, LPCWSTR pszAppName, DWORD dwDumpType,
                         MDF_CLEANUP_CALLBACK pfnCleanup, LPCWSTR pszDumpPath);
 ...
 */
DUMP_EXPORT void MiniDumpFilter_Shutdown(void);
