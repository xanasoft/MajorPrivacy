...
static WCHAR g_szDumpPath[MAX_PATH] = {0};   // Preferred output directory (/out:), empty = use temp
...
static void GenerateDumpFileName(LPWSTR szPath, LPCWSTR szDir, LPCWSTR szAppName);
...
static BOOL ParseCommandLine(LPWSTR lpCmdLine)
{
    ...
    while (*p) {
        ...
        if (*p == L'/') {
            ...
            else if (_wcsnicmp(p, L"out:", 4) == 0) {
                // Preferred output directory. May be quoted to allow spaces.
                p += 4;
                LPWSTR start;
                if (*p == L'"') {
                    p++;
                    ...
                    wcsncpy_s(g_szDumpPath, MAX_PATH, start, len);
...
static int DoUserDump(void)
{
    ...
    response.bSave = TRUE;
    // Default the save location to the caller-supplied directory (/out:) if given,
    // creating it up front; otherwise fall back to the Documents folder.
    if (g_szDumpPath[0]) {
        SHCreateDirectoryExW(NULL, g_szDumpPath, NULL);
        GenerateDumpFileName(response.szPath, g_szDumpPath, g_szAppName);
    } else {
...
//---------------------------------------------------------------------------
// DoSession0Dump - Session 0 (service): dump immediately, then offer via UI
//---------------------------------------------------------------------------
//
// When invoked as a service in session 0 we must not block waiting for user
// interaction: the crashed process has to be released quickly so the Service
// Control Manager can restart the service. Therefore we:
//   1. Write the dump to the preferred path (/out:) or, if none/failure, temp.
//   2. Terminate the crashed process so the SCM can restart it.
//   3. Only then spawn the (asynchronous) UI in the user session to let the
//      user copy/submit the already-created dump. The UI does not choose the
//      dump type in this case - it was already fixed when the dump was written.
//
static int DoSession0Dump(void)
{
    WCHAR szPipeName[64];
    ...
    BOOL bTempStaging = TRUE;   // TRUE if szDumpFile is a disposable temp file

    // Write the dump immediately (must happen while the crashed process is still
    // alive so its memory can be read). Prefer the caller-supplied directory
    // (/out:), creating it if missing, and fall back to the system temp folder
    // if that directory cannot be written.
    BOOL bDumped = FALSE;
    if (g_szDumpPath[0]) {
        // g_szDumpPath is a directory; create the tree, then write a
        // timestamped dump file inside it.
        SHCreateDirectoryExW(NULL, g_szDumpPath, NULL);
        GenerateDumpFileName(szDumpFile, g_szDumpPath, g_szAppName);
        bTempStaging = FALSE;
        bDumped = WriteDumpToFile(szDumpFile, g_dwDumpType);
    }
    if (!bDumped) {
        GenerateTempPath(szDumpFile, g_szAppName, TRUE);
        bTempStaging = TRUE;
        bDumped = WriteDumpToFile(szDumpFile, g_dwDumpType);
    }

    // Terminate the crashed process so the SCM can restart the service without
    //    waiting for the asynchronous user interaction below.
    /*{
        HANDLE hProc = OpenProcess(PROCESS_TERMINATE, FALSE, g_dwPid);
        if (hProc) {
    ...
    if (!bDumped) {
        return 1;  // Nothing was captured, nothing to offer the user.
    }

    // Spawn the UI in the user session to let the user keep/submit the dump.
    //    Any failure here is non-fatal: the dump is already safely in temp.

    // Get active console session
    activeSession = WTSGetActiveConsoleSessionId();
    if (activeSession == 0xFFFFFFFF) {
        return 0;  // No interactive session; dump remains where it was written.
    }
    ...
    // Check if user cancelled - discard the dump only if it was a temp staging
    // file. A caller-supplied path is left in place (it was written on purpose).
    if (!response.bSave && !response.bSubmit) {
        if (bTempStaging) DeleteFileW(szDumpFile);
        CloseHandle(hPipe);
        return 0;
    }
    ...
    // Copy the already-created dump to the user's chosen location.
    if (response.bSave) {
        ...
        // If the dump already lives exactly at the requested path, no copy needed.
        if (_wcsicmp(szDumpFile, response.szPath) != 0 &&
            !CopyFileW(szDumpFile, response.szPath, FALSE)) {
            result.dwResult = MDRESULT_ERROR;
            result.dwError = GetLastError();
            wcscpy_s(result.szMessage, 256, L"Failed to copy crash dump file");
        }
    }

    // Submit the dump if requested and nothing failed above.
    if (response.bSubmit && result.dwResult == MDRESULT_SUCCESS) {
        SubmitCrashDump(szDumpFile, response.szComment);
    }

    // Remove the file only if it was a temp staging copy; keep caller-supplied ones.
    if (bTempStaging) DeleteFileW(szDumpFile);
...
//---------------------------------------------------------------------------
// GenerateDumpFileName - Build a timestamped dump file path inside a directory
//---------------------------------------------------------------------------

static void GenerateDumpFileName(LPWSTR szPath, LPCWSTR szDir, LPCWSTR szAppName)
{
    WCHAR szDirTrim[MAX_PATH];
    SYSTEMTIME st;
    size_t len;

    wcscpy_s(szDirTrim, MAX_PATH, szDir);

    // Remove trailing backslash if present.
    len = wcslen(szDirTrim);
    if (len > 0 && szDirTrim[len - 1] == L'\\') {
        szDirTrim[len - 1] = L'\0';
    }
...
static INT_PTR CALLBACK CrashDumpDlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        // In session-0 UI mode the dump has already been written with a fixed
        // type; hide the type selector so the user isn't asked to choose one.
        if (g_bUiMode) {
            ShowWindow(GetDlgItem(hDlg, IDC_STATIC_TYPE), SW_HIDE);
            ShowWindow(GetDlgItem(hDlg, IDC_COMBO_TYPE), SW_HIDE);
        }
