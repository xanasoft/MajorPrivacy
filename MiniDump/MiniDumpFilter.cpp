...
static WCHAR g_szDumpPath[MAX_PATH] = {0};
                         ...
                         MDF_CLEANUP_CALLBACK pfnCleanup, LPCWSTR pszDumpPath)
{
    ...
    // Save optional preferred dump directory (used instead of the temp folder)
    if (pszDumpPath && pszDumpPath[0]) {
        wcscpy_s(g_szDumpPath, MAX_PATH, pszDumpPath);
    } else {
        g_szDumpPath[0] = L'\0';
    }
...
static LONG WINAPI MiniDumpFilter_ExceptionHandler(EXCEPTION_POINTERS* pEx)
{
    ...
    // Format: "path\MiniDump.exe" /pid:N /tid:N /ep:0xN /name:Name /type:N [/out:"path"]
    WCHAR cmdLine[1024];
    ...
    // Append the preferred output directory if one was configured. It is quoted
    // so it may contain spaces; MiniDump.exe creates the directory and falls back
    // to the temp folder if it cannot be written.
    if (g_szDumpPath[0]) {
        WCHAR szOut[MAX_PATH + 16];
        wsprintfW(szOut, L" /out:\"%s\"", g_szDumpPath);
        wcscat_s(cmdLine, ARRAYSIZE(cmdLine), szOut);
    }
